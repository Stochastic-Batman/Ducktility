...
module AverageAggregator : (Types.AGGREGATOR with type input = float and type accumulator_state = float * int and type output = float) = struct
    type input = float
...
module MinMaxAggregator : (Types.AGGREGATOR with type input = float and type accumulator_state = float * float and type output = float * float) = struct
    type input = float
...
module VolumeAggregator : (Types.AGGREGATOR with type input = int and type accumulator_state = int and type output = int) = struct
    type input = int
...
module FixedAvgEngine = (Engine.MakeMetricsEngine(FixedWindow)(AverageAggregator) : Types.EngineOutput with type input_value = AverageAggregator.input and type output_summary = AverageAggregator.output and type window_id = FixedWindow.window_identifier)

module FixedMinMaxEngine = (Engine.MakeMetricsEngine(FixedWindow)(MinMaxAggregator) : Types.EngineOutput with type input_value = MinMaxAggregator.input and type output_summary = MinMaxAggregator.output and type window_id = FixedWindow.window_identifier)

module LogVolEngine = (Engine.MakeMetricsEngine(LogarithmicWindow)(VolumeAggregator) : Types.EngineOutput with type input_value = VolumeAggregator.input and type output_summary = VolumeAggregator.output and type window_id = LogarithmicWindow.window_identifier)
...
let run_avg_sim () =
    ...
    List.iter (fun (id, res) -> Printf.printf "Bucket %d: Avg = %.2f\n" id res) (FixedAvgEngine.history state)
...
let run_minmax_sim () =
    ...
    List.iter (fun (id, (mi, ma)) -> Printf.printf "Bucket %d: Min = %.2f, Max = %.2f\n" id mi ma) (FixedMinMaxEngine.history state)
